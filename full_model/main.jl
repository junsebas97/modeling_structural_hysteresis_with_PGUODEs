#=
Implementation of the full model with the physics-guided training
...
H(x) = 0.5*(sign(x) + 1)    # Heaviside step function
...
norm(u) = u./[x_max, 1, fr_max, ε_max, x_max]    # u = [x, sign(v), fr, ε, xl]
...
# STEP 3 - full model
"""
Full model: an UODE with the equation of motion and the dynamics of the
restoring force, mass normalized dissipated energy, and largest displacement.
Here a multilayer perceptron models the restoring force.
...
5) dxl/dt     = abs(v(t))*H(|x(t)| - xl(t-))
   [mm/s]     = [mm/s]*[-]
"""
function model(du, u, θ, t, p)
    x, v, fr, ε, xl = u                  # state variables
    c = θ[1];        θ_mlp = θ[2:end]    # model parameters

    du[1] = dx  = v
    du[2] = dv  = (p(t) - c*v - fr)/(1e-6*m)
    du[3] = dfr = MLP([x, sign(v), fr, ε, xl], θ_mlp)[1]*v
    du[4] = dε  = fr*v/m
    du[5] = dxl = abs(v)*H(abs(x) - xl)
end
