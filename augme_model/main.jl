#=
Implementation of the augmented model with the physics-guided training
...
Random.seed!(87)    # 3 hidden layers of 3, 3, and 2 neurons
...
norm(u) = u./[x_max, 1, fr_max, ε_max, 1]    # u = [x, sign(v), fr, ε, s]


# STEP 2 - multilayer perceptron
MLP = FastChain((x, p) -> norm(x),         # input normalization
                FastDense(5, 3, tanh),     # first hidden layer
                FastDense(3, 3, tanh),     # second hidden layer
                FastDense(3, 2, tanh),     # third hidden layer
                FastDense(2, 2))           # output layer


# STEP 3 - augmented model
"""
Augmented model: an UODE with the equation of motion, the dynamics of the
restoring force, the mass normalized dissipated energy, and a latent variable.
Here a multilayer perceptron governs the restoring force dynamics.
...
function model(du, u, θ, t, p)
    x, v, fr, ε, s = u                               # state variables
    c = θ[1];        θ_mlp = θ[2:end]                # model parameters
    out_ANN = MLP([x, sign(v), fr, ε, s], θ_mlp)     # MLP output

    du[1] = dx  = v
    du[2] = dv  = (p(t) - c*v - fr)/(1e-6*m)
    du[3] = dfr = out_ANN[1]*v
    du[4] = dε  = fr*v/m
    du[5] = ds  = out_ANN[2]                             
end
