...
function free_vib_displ(θ, x_pred, u0, k, m, t)
    ...
    # take the damping (second parameter in the augmented model) identified by
    # the model and the initial conditions
    c        = θ[2]
    (x0, v0) = u0
...
function positive_damping(θ)
    # Extract the second parameter (damping) and return it if negative
    return max(0.0, -θ[2])
end
