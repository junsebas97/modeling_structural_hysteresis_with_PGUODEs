...
function good_initialize(example, θ_syst, t_fv)
    while true
        ...
        θ = [                            randn();    # initial condition of s(t)
                                    abs(randn());    # damping
...
function print_validation_results(θ_model, example, θ_syst, t_fv, Fv)
    ...
    ω0 = sqrt(1e6*k/m)
    ξ  = min(1e6*θ_model[2]/(2*m*ω0), 0.999)
    ωD = ω0*sqrt(1 - ξ^2)
    ...
    (x0, v0) = (0.95*uy, 0.0)
    fv_ed    = Dict("u0"   => [x0, v0, k*x0, k*(x0^2)/2],
                    "t"    => t_fv,
                    "p(t)" => t -> 0.0*t,
